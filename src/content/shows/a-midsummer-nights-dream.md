---
audition_date: 2027-03-24
title: A Midsummer Night's Dream
eyebrow: 2027 Studio Show
start_date: 2027-05-07
end_date: 2027-05-08
date_display: May 2027
venue: PYT Studio, Old Middlefield Way
runtime_minutes: 60
poster: /uploads/midsummer.jpg
synopsis: Shakespeare’s comedy of love, magic, and fairy mischief is a perfect
  fit for the Studio Show format — a small, tight-knit cast working together on
  material that demands real precision, playfulness, and trust. Heightened
  language, physical storytelling, ensemble timing — the skills you build in
  this room will stay with you long after closing night. This is the kind of
  creative experience that bonds a group of performers together and genuinely
  levels up your acting.
audition_display: March 24, 2027
---
