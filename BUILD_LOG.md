...
Per-show double-cast performance schedule (2026-06-09)
Goal (PM): Mainstage shows are always double cast, but the external ticketing site (tickets.mvcpa.com) refuses to label which cast performs on which date — it only shows venue codes (e.g. Lancaster/York). Families buying tickets can't tell which cast they're getting. Add a schedule on each show page listing every performance with its date, time, cast name, and a buy link straight to that exact performance. Mainstage-only (double cast); must NOT appear for Stories on Stage, Studio Shows, etc.
Decisions with PM: (1) Lives ON the existing show page (/shows/<slug>) as a self-contained "Performances & casts" section, NOT a separate page per show — fewer routes, where families already land. (2) Per-performance buy links (one per row), not the show's master link — clearer for buyers, PM confirmed no way around it. (3) Rows shown in entry order with a cast LABEL per row (date · time · cast · Buy button), NOT grouped under cast headings — reads in date order, how families pick ("I want Saturday"), best on mobile. (4) Data entered by hand as a repeatable CMS list per show (PM chose this over any attempt to auto-pull from the MVCPA site).
Built (3 pieces):
src/content.config.ts (EDIT) — added to the shows schema: cast_a_name, cast_b_name (optional strings, CMS-hint helpers only), and performances — an optional, tolerant array of { date_label, time_label, cast, buy_url } (all subfields optional, outer .catch(undefined)), modeled exactly on the existing cast-pages resources list.
src/pages/shows/[slug].astro (EDIT) — renders a "Performances & casts" section after the synopsis. Filters performances to rows that have at least date_label AND cast (a half-filled row is dropped, not rendered blank); if none survive, the whole section is hidden. Each row: date + optional time, the cast name, and either a "Buy tickets" button (if buy_url present) or a non-clickable "On sale soon" pill (if blank). Mobile-first styles using existing tokens; button goes full-width under ~520px.
public/admin/config.yml (EDIT — insert fields into the shows collection) — added cast_a_name, cast_b_name, and a performances list widget (subfields: date_label, time_label, cast, buy_url) with plain-language hints, after cast_page_url. List summary shows "{{date_label}} {{time_label}} — {{cast}}".
Tolerance / Rule 2: no src/content/** files touched. New schema fields are all optional, so existing live show files (which lack them) build fine and show no schedule block — only shows where the PM enters performances display the section. This is the "handle the field being absent" requirement from the handover.
Tested (sandbox COULD reach GitHub this session — real clone + build): npm run build passes, 20 pages (unchanged — this adds a section, not a route). Empty case verified: Wizard of Oz (no performances) renders NO schedule section (0 <section> elements; only the inert CSS sits in the stylesheet). Populated case verified with temporary sample data on Wizard (4 good rows + 1 deliberately broken row with no date/cast): exactly 4 rows rendered, broken row silently dropped, its URL did not leak; 3 rows with links showed "Buy tickets", 1 without showed "On sale soon"; both cast names rendered correctly. Temporary sample data was then REMOVED and the file restored to original (git diff clean). config.yml re-validated as YAML after the edit; performances subfields confirmed to match the page one-to-one.
NOT verified (needs PM): the live visual look on phone + desktop (Claude can't screenshot — always the PM's check); that the new "Performance schedule" fields appear in /admin (needs the PM's hard-refresh of /admin after the config.yml change).
Still open / next: PM uploads the 3 changed files; after the config.yml change, hard-refresh /admin; then enter performances for any currently-selling Mainstage show (e.g. SIX) and visually check one show page on phone + desktop.
